...
describe 'API' do
  ...
  context 'search by title' do
    ...
    it 'should exactly match' do
      get URI.escape('/api/soupcms-api-test/posts?filters=title&title="My first blog post"')
    ...
    it 'should match with give regular expression' do
      get URI.escape('/api/soupcms-api-test/posts?filters=title&title=/post/')
    ...
    it 'should match multiple filters' do
      get URI.escape('/api/soupcms-api-test/posts?filters[]=title&filters[]=category&title=/post/&category="cooking"')
    ...
    it 'should match filter with multiple value as or condition' do
      get URI.escape('/api/soupcms-api-test/posts?filters[]=title&filters[]=category&title=/post/&category[]="cooking"&category[]="technical"')
    ...
    it 'should match filter with integer value' do
      get URI.escape('/api/soupcms-api-test/posts?filters=rank&rank=25')
